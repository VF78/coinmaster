...
Updated: 2026-04-27 11:25 Europe/Madrid
Status: ACTIVE / #65 native Freqtrade Stage 1 is deployed in dry-run; Telegram notifications enabled and dry-run DB persisted
...
Checks / commit / deploy / push: latest checks passed (`npm run check`, `npm run invariants:trading-rules`, `npm run build`, deploy script `bash -n`); changes committed+pushed to `origin/freqtrade-stage1-migration`; production deploy rerun after commit; Freqtrade service restarted, dry-run started, API verified healthy (`dry_run=true`, `state=running`, Telegram RPC listening, no locks/trades).
