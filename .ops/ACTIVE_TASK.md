...
Updated: 2026-04-27 12:35 Europe/Madrid
Status: ACTIVE / #65 native Freqtrade Stage 1 dry-run plus #67 Freqtrade-native Radar policy bridge deployed neutral-by-default
...
Branch / HEAD / origin/main / deploy commit / divergence: freqtrade-stage1-migration / c698f1d8122733b18fd16ceb45b9f791d27a3a66 committed+pushed / origin/main f6f2279cd10eb97223d20cbf958a6c4e2f93737a / deploy marker c698f1d8122733b18fd16ceb45b9f791d27a3a66 / origin/main...HEAD ahead
...
- Freqtrade Strategy now has neutral-by-default Radar policy reader/cache deployed; stale/missing/invalid policy is ignored, global/pair/direction blocks are guarded, and risk_multiplier is clamped 0..1 and applied after existing caps. `/var/lib/coinmaster/freqtrade/radar_policy.json` is currently missing by design, so behavior is neutral/unmodified.
...
Next exact step: implement/choose Radar producer that atomically writes `radar_policy.json`; then continue observing dry-run until fresh signals/trades appear.
Checks / commit / deploy / push: Radar bridge checks passed (`python3 -m py_compile CoinMasterStrategy.py`, docker `invariants_radar_policy.py` 9/9, `npm run check`, `npm run build`, deploy smoke); commit c698f1d pushed; production deploy rerun after commit; Freqtrade service restarted, dry-run started, API verified healthy (`dry_run=true`, `state=running`, Telegram RPC listening, no locks/trades, `radar_policy=missing_neutral`).
