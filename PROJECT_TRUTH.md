...
Последнее обновление: 2026-02-15 15:46 Europe/Madrid
...
## Приоритизация (owner update 2026-02-14/15)
...
- Операционный фикс после инцидента: hard-lock модели на `openai-codex/gpt-5.3-codex` + корректировка cron-логики «только подтверждённый прогресс и обязательный апдейт GitHub Project перед completion-сообщением».
...
- UI rename/label polishing (без изменения цветовой схемы и общего визуального стиля):
  - `Trading copilot` -> `Coinmaster24`;
  - `Account overview (real, Hyperliquid)` -> `Account overview (Hyperliquid + CONNECTED icon)`;
  - удалить `Status: CONNECTED`;
  - `Manual confirmation: ON • Limits: 30 USDC / 10x` перенести в блок `Execution controls` (вниз).
- Account overview improvements:
  - убрать кнопку refresh, сделать автообновление каждые 5 секунд;
  - объединить weekly/monthly P&L и добавить daily (по умолчанию daily, далее переключение daily/weekly/monthly);
  - удалить `BTC mark`;
  - `Open exposure` -> `Open positions`.
- Positions flow:
  - `Live open positions (exchange)` -> `Live open positions`;
  - добавить блок `Positions to confirm` (сигналы, ожидающие ручного подтверждения, явный риск-warning, перенос в `Live open positions` после подтверждения открытия и установки SL/TP).
- Sidebar UX:
  - слева разделы списком (по аналогии `myshopai`), не кнопками;
  - добавить раздел `Trading Rules` после `History`.
- Trading Rules section (параметризация стратегии и риска):
  - выбор монет BTC/ETH/SOL + пропорции капитала по монетам;
  - правила входа/выхода (таймфреймы поглощения, правила FVG/retest, дефолт 50% retrace);
  - risk/MM настройки (макс суммарное плечо портфеля + enforcement при каждом открытии, макс дневная просадка на авто-закрытие);
  - рекомендации на сделку (рекомендуемый риск/плечо, дефолт SL%, 3 TP% с редактированием);
  - режим открытия (auto или manual confirm), кнопка `Apply` с рисковым подтверждением.
- Telegram execution notifications:
  - после `Apply` включить надёжный мониторинг триггеров;
  - при триггере — уведомление о необходимости подтверждения или об авто-открытии;
  - после факта открытия — отдельное подтверждающее уведомление.

### Post-launch backlog (делаем после старта прода)
- Кодовый аудит + удаление мёртвого кода + performance оптимизация + упрощение архитектуры (анти-spaghetti).
