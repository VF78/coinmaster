...
export function App() {
  ...
  return <div className="app-shell nautilus-app-shell">
    <header className="app-topbar nautilus-topbar"><div><h1>Coinmaster24</h1><p className="muted">Nautilus operator workspace · {active.label}</p>{sessionError ? <p role="alert">{sessionError}</p> : null}</div><div className="rules-btn-group"><Badge tone="neutral">SANDBOX · READ ONLY</Badge><span className="muted">{operator}</span><Button variant="secondary" onClick={() => { void logoutGuiSession().catch((error: unknown) => setSessionError(error instanceof Error ? error.message : 'Sign out failed.')); }}>Sign out</Button></div></header>
