  ...
  return <main className="terminal-layout nautilus-runtime-workspace">
    <Card className="trader-hero runtime-hero" title="Account overview" actions={<Badge tone={toneForProjection}>{state ? `${state.projection_state} · ${state.process_state}` : loading ? 'LOADING' : 'UNAVAILABLE'}</Badge>}>
      ...
      <div className="runtime-account-grid">
        ...
        <Stat label="Working orders" value={projectionReady ? String(orderRows.length) : "UNKNOWN"} />
      </div>
    ...
    </Card><Card className="positions-board" title="Positions" actions={<Badge tone="neutral">SANDBOX</Badge>}><DataTable rows={positionRows} emptyText={projectionReady ? "No positions are open." : "Position state unavailable."} mobileTitle={(row) => row.instrument_id} mobileSubtitle={(row) => `${row.signed_quantity} · mark ${row.mark ?? 'UNKNOWN'} · Sandbox`} columns={[{ key: 'instrument', header: 'Market', render: (row) => row.instrument_id }, { key: 'quantity', header: 'Position size', render: (row) => row.signed_quantity }, { key: 'mark', header: 'Current mark', render: (row) => row.mark ?? 'UNKNOWN' }, { key: 'provenance', header: 'Execution', render: (row) => row.provenance }]} /></Card></section>
    <section className="runtime-history-grid"><Card className="orders-board" title="Working orders" actions={<Badge tone="neutral">SANDBOX</Badge>}><DataTable rows={orderRows} emptyText={projectionReady ? "No working orders." : "Order state unavailable."} mobileTitle={(row) => row.instrument_id ?? row.client_order_id} mobileSubtitle={(row) => `${row.reduce_only ? 'TP / reduce-only' : 'Working order'} · ${row.quantity ?? 'UNKNOWN'} @ ${row.price ?? 'UNKNOWN'}`} columns={[{ key: 'instrument', header: 'Market', render: (row) => row.instrument_id ?? 'UNKNOWN' }, { key: 'purpose', header: 'Order', render: (row) => row.reduce_only === true ? 'TP / reduce-only' : row.reduce_only === false ? 'Working order' : 'UNKNOWN' }, { key: 'side-type', header: 'Side / type', render: (row) => `${row.side ?? 'UNKNOWN'} / ${row.order_type ?? 'UNKNOWN'}` }, { key: 'price', header: 'Limit price', render: (row) => row.price ?? 'UNKNOWN' }, { key: 'qty-leaves', header: 'Quantity / leaves', render: (row) => `${row.quantity ?? 'UNKNOWN'} / ${row.leaves_quantity ?? 'UNKNOWN'}` }, { key: 'reduce', header: 'Reduce only', render: (row) => row.reduce_only === null || row.reduce_only === undefined ? 'UNKNOWN' : row.reduce_only ? 'YES' : 'NO' }, { key: 'provenance', header: 'Execution', render: (row) => row.provenance }]} /></Card>
    <Card className="history-board" title="Trade history" actions={<Badge tone="neutral">NATIVE EVIDENCE</Badge>}>
      <p className="empty-state-title">{hasNativeActivity ? "Verified trade details unavailable" : "No virtual trades yet"}</p><p className="muted">Cumulative realized PnL and fees above come from the native account projection. This view does not reconstruct per-fill PnL.</p>
      {hasNativeActivity ? <p className="muted">Native activity exists; per-fill details are not included in this projection.</p> : null}
    </Card></section>
    <details className="nautilus-diagnostics"><summary>Diagnostics and source identity</summary><div className="terminal-layout"><p className="muted">Instance: <code>{state?.instance_id ?? 'hl-stageg-testnet'}</code> · reconciliation: {state?.reconciliation ?? 'UNKNOWN'} · warmup: {state?.warmup.state ?? 'UNKNOWN'} ({state?.warmup.rows ?? 'UNKNOWN'} rows).</p><p className="muted">Candidate: {state?.hashes.candidate_sha256 ?? 'UNKNOWN'} · strategy: {state?.hashes.strategy_sha256 ?? 'UNKNOWN'} · policy: {state?.hashes.execution_policy_sha256 ?? 'UNKNOWN'}.</p><p className="muted">Account: {state?.account.status ?? "UNAVAILABLE"} | initial margin: {state?.account.im ?? 'UNKNOWN'} · maintenance margin: {state?.account.mm ?? 'UNKNOWN'} · activity cursor: {state?.event_cursor ?? 'UNKNOWN'}.</p><p className="muted">Warnings: {state?.warnings.join(' · ') || 'NONE'} · API detail: {message || 'NONE'}.</p><DataTable rows={eventRows} emptyText="No native activity recorded." mobileTitle={(row) => row.kind} columns={[{ key: 'cursor', header: 'Cursor', render: (row) => row.cursor }, { key: 'kind', header: 'Kind', render: (row) => row.kind }, { key: 'event', header: 'Event reference', render: (row) => row.event_id }]} /></div></details>
    {message ? <p role="alert">Runtime account data is unavailable. Refresh to retry.</p> : null}
  </main>;
}
