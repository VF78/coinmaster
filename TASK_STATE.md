...
Current: P0 partial (P0.1 verified/closed; P0.2 open). P1 native fixture has fills/funding/basic margin plus event-ID/transfer-total journal; restart reconciliation and tier deductions remain open.
...
Tested: `uv lock --check`; `uv sync --group dev`; CPython 3.12.13; `nautilus=1.231.0`; adapters expose funding plus `QueryAccount`/`AccountState`; `.venv/bin/python -m pytest -q` (8 passed; Pandas deprecation warnings).
